...
describe('SubscriptionList', () => {
  ...
  it('renders one SubscriptionListItem for each subscription contract', async () => {
    ...
    expect(screen.getAllByText('View subscription details')).toHaveLength(
      contractCount,
    );
  });
  ...
  describe('Success toasts', () => {
    ...
    describe('<ResumeSubscriptionModal />', () => {
      ...
      it('is passed last the last order price and pricebreakdown total as next order price', async () => {
        mockSubscriptions({
          contractCount: 1,
          contractStatus: 'PAUSED' as SubscriptionContractSubscriptionStatus,
          priceBreakdownTotal: '200',
          lastOrderTotal: '100',
        });
        ...
        expect(
          screen.getByText(
            /The price for this subscription has changed and you will be charged \$200\.00 per upcoming order/,
          ),
        ).toBeInTheDocument();
      });
    });
