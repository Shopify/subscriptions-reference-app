...
describe('SubscriptionListItem', () => {
  ...
  it('renders the delivery frequency', async () => {
    ...
    expect(screen.getByText('Delivery every 3 weeks')).toBeInTheDocument();
  });
  ...
  it('displays an invalid payment warning when the payment method is invalid', async () => {
    await mountWithAppContext(
      <SubscriptionListItem {...defaultProps} hasInvalidPaymentMethod />,
    );
