...
describe('SellingPlanDetails', () => {
  ...
  describe('create page', () => {
          ...
          variables: {
            input: {
              sellingPlansToCreate: [
                {
                  name: 'Delivery every 2 weeks, 15% off',
                  billingPolicy: {
                    recurring: {
                      interval: 'WEEK',
                      intervalCount: 2,
        ...
        CreateSellingPlanGroupMutation,
        {
          variables: {
            input: {
              sellingPlansToCreate: [
                {
                  name: 'Delivery every week, $5.00 off',
                  billingPolicy: {
                    recurring: {
                      interval: 'WEEK',
                      intervalCount: 1,
                ...
                {
                  name: 'Delivery every year, $2.00 off',
                  billingPolicy: {
                    recurring: {
                      interval: 'YEAR',
                      intervalCount: 1,
    ...
    it('creates a selling plan with flat rate based selling plans', async () => {
      await mountSellingPlanDetails({id: 'create'});
      ...
      // Select amount off discount type
      await userEvent.click(screen.getByRole('radio', {name: 'Flat rate'}));
      ...
      await userEvent.type(screen.getAllByLabelText('Flat rate')[1], '20');
      ...
      await userEvent.type(screen.getAllByLabelText('Flat rate')[2], '45');
        ...
        CreateSellingPlanGroupMutation,
        {
          variables: {
            input: {
              sellingPlansToCreate: [
                {
                  name: 'Delivery every 3 weeks, $20.00 flat rate',
                  billingPolicy: {
                    recurring: {
                      interval: 'WEEK',
                      intervalCount: 3,
                ...
                {
                  name: 'Delivery every 5 months, $45.00 flat rate',
                  billingPolicy: {
                    recurring: {
                      interval: 'MONTH',
                      intervalCount: 5,
        ...
        CreateSellingPlanGroupMutation,
        {
          variables: {
            input: {
              sellingPlansToCreate: [
                {
                  name: 'Delivery every week, £25.00 off',
                },
              ],
            },
          },
  ...
  describe('edit page', () => {
    ...
    it('displays price selling plan inputs when returned from graphql', async () => {
      ...
      expect(screen.getAllByLabelText('Flat rate')[1]).toHaveValue(
                ...
                sellingPlansToCreate: [
                  {
                    name:
                      adjustmentType === 'PRICE'
                        ? 'Delivery every 3 months, $20.00 flat rate'
                        : `Delivery every 3 months, ${expectedCreatedAmount} off`,
                    billingPolicy: {
                      recurring: {
                        interval: 'MONTH',
                        intervalCount: 3,
                ...
                sellingPlansToUpdate: [
                  {
                    ...
                    name:
                      adjustmentType === 'PRICE'
                        ? 'Delivery every 2 years, $15.00 flat rate'
                        : `Delivery every 2 years, ${expectedUpdatedAmount} off`,
                    billingPolicy: {
                      recurring: {
                        interval: 'YEAR',
                        intervalCount: 2,
  ...
  describe('Summary card', () => {
    describe('when there is one delivery option', () => {
      it('displays percentage discount info', async () => {
        ...
        expect(screen.getByText('Delivery every 2 weeks, 33% off'));
      });
      ...
      it('displays amount off discount info', async () => {
        ...
        expect(screen.getByText('Delivery every 3 months, $5.00 off'));
      });

      it('displays flat rate discount info', async () => {
        const mockGraphQLResponses = {
          ...defaultGraphQLResponses,
          SellingPlanGroup: {
            data: {
        ...
        expect(screen.getByText('Delivery every 2 weeks, $10.00 flat rate'));
      });
    });
    ...
    it('Renders expected modal and content', async () => {
      ...
      expect(
        await screen.findByText(
          "Are you sure you want to delete the Selling Plan subscribe-2-save? This can't be undone.",
        ),
      ).toBeInTheDocument();
    });
