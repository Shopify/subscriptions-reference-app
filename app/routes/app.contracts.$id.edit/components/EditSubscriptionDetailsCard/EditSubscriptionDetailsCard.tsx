...
export function EditSubscriptionDetailsCard({
  ...
  const removeLine = (lineIndex: number) => {
    form.setValue(
      'lines',
      form.value('lines').filter((_, index) => index !== lineIndex),
    );
  };
  ...
  */
  const selectedItemsForProductPicker = form
    .value('lines')
  ...
  async function selectProducts(
    ...
    selectedItems.selection.forEach((selectedItem) => {
      /*
    ...
    const linesToKeep = [...form.value('lines'), ...newLines].filter(
  ...
  return (
    <Card>
      <BlockStack gap="400">
        ...
        {form.value('lines').map((line, index) => (
          <EditSubscriptionLine
            key={index}
            line={line}
            index={index}
            removeLine={form.value('lines').length > 1 ? removeLine : undefined}
      ...
      <EditSubscriptionLineInputs lines={form.value('lines')} />
    </Card>
  );
}
